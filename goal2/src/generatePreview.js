...
function generatePreviewHTML(collision, props, meta) {
  const data = JSON.stringify({ collision, props, meta });
...
<div id="legend">
  ...
  <div class="group-label">碰撞组 1 (红)</div>
  ...
  <div class="group-label">碰撞组 2 (绿)</div>
...
function addType1(arr, color, group) {
...
function addType2(arr, color, group) {
  if (!arr.length) return;
  const mat = makeMaterial(color);
  for (const d of arr) {
    const length = d[0];
...
function addType3(arr, color, group) {
  if (!arr.length) return;
...
addType1(col.collisionData1.type1, COLORS.cd1_t1, collisionGroup);
addType2(col.collisionData1.type2, COLORS.cd1_t2, collisionGroup);
addType3(col.collisionData1.type3, COLORS.cd1_t3, collisionGroup);
addType1(col.collisionData2.type1, COLORS.cd2_t1, collisionGroup);
addType2(col.collisionData2.type2, COLORS.cd2_t2, collisionGroup);
addType3(col.collisionData2.type3, COLORS.cd2_t3, collisionGroup);
...
document.getElementById('btn-props').addEventListener('click', (e) => {
  const active = !propGroup.visible;
  propGroup.visible = active;
