// ==UserScript==
// @name         Tanki Map Simplifier v1.15.3
// @namespace    https://github.com/0d00no0721/TOandDDD
// @version      1.15.3
// @description  Intercept texImage2D + createImageBitmap + Image.src + fetch + XHR + WebGL uniforms. Custom image replacement (single image, baked alpha + opacity slider, no-tiling override) with HSL fallback, hide billboards. Ctrl+Shift+M toggle.
...
(function() {
  ...
  var TILING_KEY = 'Tanki_Simplified_NoTiling';
  ...
  // ═══ UV tiling override (force "one full image per face", no repetition) ═══
  // Hook getUniformLocation to build a loc->name map, then override setter calls
  // for tiling-related uniforms so the texture shows once per face instead of
  // repeating in small tiles. Toggle via UI "No-Tiling" checkbox.
  var TILING_NAMES = ['uvScale', 'mainTiling', 'textureScaleAndOffset', 'uvTransform',
                      'uvOffset', 'textureTranslation', 'uvCorrection', 'textureCorrection'];
  var tilingEnabled = (function() { try { return localStorage.getItem(TILING_KEY) !== 'false'; } catch(e) { return true; } })();
  function setTilingEnabled(v) { try { localStorage.setItem(TILING_KEY, String(v)); } catch(e) {} }
  function getTilingEnabled() { try { return localStorage.getItem(TILING_KEY) !== 'false'; } catch(e) { return true; } }

  var locToName = new Map();        // WebGLUniformLocation -> uniform name (string)
  var tilingLogged = {};            // name -> true (logged once)
  var tilingHooksInstalled = false;

  function matchesTilingName(name) {
    if (!name) return null;
    var lower = String(name).toLowerCase();
    for (var i = 0; i < TILING_NAMES.length; i++) {
      if (lower === TILING_NAMES[i].toLowerCase()) return TILING_NAMES[i];
    }
    return null;
  }

  // Decide replacement value for a tiling uniform based on its name semantics.
  // Returns an array (for vec setters) or number (for scalar). Caller adapts by arity.
  function tilingReplacement(name, arity) {
    var lower = String(name).toLowerCase();
    if (lower.indexOf('offset') !== -1 || lower.indexOf('translation') !== -1) {
      // zero offset
      if (arity === 1) return 0;
      if (arity === 2) return [0, 0];
      if (arity === 3) return [0, 0, 0];
      if (arity === 4) return [0, 0, 0, 0];
      return 0;
    }
    if (lower.indexOf('scale') !== -1 || lower.indexOf('tiling') !== -1) {
      // scale = 1 (no repetition)
      if (arity === 1) return 1;
      if (arity === 2) return [1, 1];
      if (arity === 3) return [1, 1, 1];
      if (arity === 4) return [1, 1, 1, 1];
      return 1;
    }
    if (lower.indexOf('transform') !== -1 || lower.indexOf('scaleandoffset') !== -1) {
      // combined scale+offset: identity = scale 1, offset 0
      if (arity === 2) return [1, 0];
      if (arity === 4) return [1, 1, 0, 0];
      if (arity === 1) return 1;
      return [1, 1, 0, 0];
    }
    if (lower.indexOf('correction') !== -1) {
      // best guess: identity (1). Adapts per-arity.
      if (arity === 1) return 1;
      if (arity === 2) return [1, 1];
      if (arity === 4) return [1, 1, 1, 1];
      return 1;
  ...
  function logTilingOverride(name, oldVal, newVal) {
    if (tilingLogged[name]) return;
    tilingLogged[name] = true;
    log('NoTiling: "' + name + '" ' + (oldVal === undefined ? '' : JSON.stringify(oldVal)) + ' -> ' + JSON.stringify(newVal));
  }

  function installTilingHooks(Ctor, versionTag) {
    ...
    if (origGetLoc) {
      Ctor.prototype.getUniformLocation = function(program, name) {
        ...
        if (loc && name) {
          try {
            if (matchesTilingName(name)) locToName.set(loc, String(name));
          } catch(e) {}
        }
        return loc;
      };
    }
    // Scalar and vector setters. arity inferred from the uniform type each setter handles.
    var setters = [
      { fn: 'uniform1f', arity: 1 },
      { fn: 'uniform2f', arity: 2, vecArgs: true },
      { fn: 'uniform2fv', arity: 2, vecArr: true },
      { fn: 'uniform3f', arity: 3, vecArgs: true },
      { fn: 'uniform3fv', arity: 3, vecArr: true },
      { fn: 'uniform4f', arity: 4, vecArgs: true },
      { fn: 'uniform4fv', arity: 4, vecArr: true }
    ];
    setters.forEach(function(s) {
      var orig = Ctor.prototype[s.fn];
      if (!orig) return;
      Ctor.prototype[s.fn] = function(loc) {
        if (!getTilingEnabled() || !loc) return orig.apply(this, arguments);
        var name = locToName.get(loc);
        if (!name || !matchesTilingName(name)) return orig.apply(this, arguments);
        var rep = tilingReplacement(name, s.arity);
        if (rep === null) return orig.apply(this, arguments);
        // Build new args: location + replacement values
        var args;
        if (s.vecArr) {
          // uniformNfv(loc, valueArray)
          args = [loc, Array.isArray(rep) ? new Float32Array(rep) : new Float32Array([rep])];
        } else if (s.vecArgs) {
          // uniformNf(loc, x, y, ...)
          args = [loc].concat(Array.isArray(rep) ? rep : [rep]);
        } else {
          // uniform1f(loc, x)
          args = [loc, typeof rep === 'number' ? rep : rep[0]];
        }
        logTilingOverride(name, Array.prototype.slice.call(arguments, 1), rep);
        return orig.apply(this, args);
      };
    });
    logStep(7, 'NoTiling hooks installed (' + versionTag + '): ' + TILING_NAMES.join(', '));
  }

  installTilingHooks(WebGLRenderingContext, 'WebGL1');
  if (typeof WebGL2RenderingContext !== 'undefined') installTilingHooks(WebGL2RenderingContext, 'WebGL2');
  tilingHooksInstalled = true;
  ...
  function updateStatus() {
    ...
    var imgLine = active ? '<div style="color:' + (customImageReady ? '#76ff33' : '#8a9ba8') + ';font-size:10px;margin-top:2px;">IMG: ' + (customImageReady ? ('ON ' + customImageWidth + 'x' + customImageHeight + ' a' + IMG_ALPHA.toFixed(2)) : 'OFF (HSL fallback)') + ' | NoTiling: ' + (getTilingEnabled() ? 'ON' : 'OFF') + '</div>' : '';
  ...
  function createConfigPanel() {
    ...
    configPanel.innerHTML =
      ...
      '<div style="margin-bottom:6px;font-size:10px;color:#aaa;">' +
        '<label style="cursor:pointer;display:flex;align-items:center;gap:4px;"><input id="ms-notiling-checkbox" type="checkbox" checked style="accent-color:#76ff33;cursor:pointer;">No-Tiling (one image per face)</label>' +
      '</div>' +
    ...
    var notilingCheckbox = document.getElementById('ms-notiling-checkbox');
    if (notilingCheckbox) {
      notilingCheckbox.checked = getTilingEnabled();
      notilingCheckbox.addEventListener('change', function(e) {
        setTilingEnabled(e.target.checked);
        log('No-Tiling ' + (e.target.checked ? 'ENABLED' : 'DISABLED') + ' (takes effect on next texture reload / map reload)');
        updateStatus();
      });
    }
