// ==UserScript==
// @name         Tanki Map Simplifier v1.15.1
// @namespace    https://github.com/0d00no0721/TOandDDD
// @version      1.15.1
// @description  Intercept texImage2D + createImageBitmap + Image.src + fetch + XHR. Custom image replacement (single image for all textures) with HSL fallback, hide billboards (vegetation keywords + pattern). Ctrl+Shift+M toggle.
...
(function() {
  ...
  // (resampled via interpolation to fit CUSTOM_MAX_DIM on its longest side).
  // When not ready, the v1.14 HSL gradient path is used as fallback.
  var CUSTOM_MAX_DIM = 256;
  var customImageDataURL = null;   // processed PNG dataURL (stored in localStorage)
  var customImageReady = false;    // derived resources are ready for use
  var customImageCanvas = null;    // resampled canvas (for texImage2D / createImageBitmap)
  var customImageBytes = null;     // PNG bytes (for fetch / XHR response body)
  var customImageBlobUrl = null;   // blob: URL (for Image.src / setAttribute)
  ...
  // Build all derived resources from a ready canvas + its PNG dataURL.
  function deriveCustomResources(canvas, dataURL) {
    customImageCanvas = canvas;
    customImageWidth = canvas.width;
    customImageHeight = canvas.height;
    customImageDataURL = dataURL;
    customImageBytes = dataUrlToBytes(dataURL);
  ...
  function preloadCustomImage() {
    ...
    img.onload = function() {
      try {
        var canvas = resampleToCanvas(img, CUSTOM_MAX_DIM);
        deriveCustomResources(canvas, dataURL);
        log('Custom image ready: ' + customImageWidth + 'x' + customImageHeight);
  ...
  function updateStatus() {
    ...
    var imgLine = active ? '<div style="color:' + (customImageReady ? '#76ff33' : '#8a9ba8') + ';font-size:10px;margin-top:2px;">IMG: ' + (customImageReady ? ('ON ' + customImageWidth + 'x' + customImageHeight) : 'OFF (HSL fallback)') + '</div>' : '';
    ...
    reader.onload = function() {
      ...
      img.onload = function() {
        try {
          ...
          setStoredCustomImg(processedDataURL);
          releaseCustomImage();
          deriveCustomResources(canvas, processedDataURL);
          log('Custom image uploaded: ' + customImageWidth + 'x' + customImageHeight);
          updateStatus(); updateConfigUI();
        } catch(e) {
          logWarn('Custom image processing failed: ' + e.message);
          updateStatus(); updateConfigUI();
  ...
  function updateConfigUI() {
    ...
    if (customImageReady && customImageDataURL) {
      configThumb.innerHTML = '<img src="' + customImageDataURL + '" style="width:100%;max-width:200px;border:1px solid rgba(118,255,51,0.3);border-radius:4px;display:block;">';
      configStatus.textContent = 'IMG: ON ' + customImageWidth + 'x' + customImageHeight;
