// ==UserScript==
// @name         Tanki Map Simplifier v1.15.4-discovery
// @namespace    https://github.com/0d00no0721/TOandDDD
// @version      1.15.4-discovery
// @description  READ-ONLY discovery probe: records per-program uniforms + draw counts + mat4 matrices to determine if UV-override-by-position is feasible. Does NOT modify any uniform/draw (cannot break rendering). Use once, send console logs to dev.
...
(function() {
  ...
  var VERSION = '1.15.4-discovery2';
  var VERSION_FULL = '1.15.4-discovery2';
  ...
  var LOG_URL = 'http://localhost:3458/log';
  ...
  // ═══ Discovery probe (READ-ONLY: never overrides, only records) ═══
  // Goal: determine if world-geometry draws are per-prop (one modelToWorldMatrix each)
  // or batched (one matrix, many props). This decides whether UV-override-by-position
  // (tile one image across all horizontal surfaces) is feasible via uniforms.
  // SAFETY: every hook calls the original unchanged; we only READ state. Cannot break rendering.
  var discoveryPrograms = new Map();   // program -> {index, uniformNames:Set, mat4:{}, drawCount, shaderUniforms:[]}
  var progIndexCounter = 0;
  var locToProgName = new Map();       // WebGLUniformLocation -> {program, name}
  var discoveryWindowActive = false;   // draw-counting only active post-map for 2s
  var discoveryFlushed = false;
  var WORLD_MAT4_NAMES = ['modelToWorldMatrix', 'mvp', 'objectToViewMatrix', 'modelToViewMatrix', 'worldMatrix'];

  function getOrMakeProg(program) {
    if (!discoveryPrograms.has(program)) {
      discoveryPrograms.set(program, { index: ++progIndexCounter, uniformNames: new Set(), mat4: {}, drawCount: 0, shaderUniforms: [] });
    }
    return discoveryPrograms.get(program);
  }

  function extractUniformsFromSource(src) {
    if (!src) return [];
    var re = /uniform\s+(?:highp|mediump|lowp)?\s*(\w+)\s+(\w+)\s*(?:\[[^\]]*\])?\s*;/g;
    var out = []; var m;
    while ((m = re.exec(src)) !== null) { out.push(m[1] + ' ' + m[2]); }
    return out;
  }

  function classifyProg(info) {
    var names = info.shaderUniforms.map(function(s){ return s.split(' ')[1]; });
    var has = function(n){ return names.indexOf(n) !== -1; };
    if (has('modelToWorldMatrix') || has('mainTiling') || has('uvScale') || has('lightmapTexture')) return 'world';
    if (has('u_screenSize') || has('u_pos') || has('u_size') || has('u_color') || has('u_fontTexture')) return 'ui';
    if (has('time') || names.some(function(n){ return /particle|sprite|emission/i.test(n); })) return 'effects';
    return 'unknown';
  }

  function mat4Key(v) { return Math.round(v[12]*10) + ',' + Math.round(v[13]*10) + ',' + Math.round(v[14]*10); }

  function installDiscoveryHooks(Ctor, tag) {
    if (!Ctor || !Ctor.prototype) return;
    var origLink = Ctor.prototype.linkProgram;
    Ctor.prototype.linkProgram = function(program) {
      origLink.call(this, program);
      try {
        var info = getOrMakeProg(program);
        var shaders = this.getAttachedShaders(program);
        var all = [];
        for (var i = 0; i < shaders.length; i++) {
          var src = this.getShaderSource(shaders[i]);
          all = all.concat(extractUniformsFromSource(src));
        }
        info.shaderUniforms = all;
      } catch(e) {}
    };
    var origGetLoc = Ctor.prototype.getUniformLocation;
    Ctor.prototype.getUniformLocation = function(program, name) {
      var loc = origGetLoc.call(this, program, name);
      if (loc !== null && name) {
        try {
          locToProgName.set(loc, { program: program, name: String(name) });
          getOrMakeProg(program).uniformNames.add(String(name));
        } catch(e) {}
      }
      return loc;
    };
    var origUM4 = Ctor.prototype.uniformMatrix4fv;
    Ctor.prototype.uniformMatrix4fv = function(loc, transpose, value) {
      try {
        var info = locToProgName.get(loc);
        if (info && value && value.length >= 16 && WORLD_MAT4_NAMES.indexOf(info.name) !== -1) {
          var pinfo = getOrMakeProg(info.program);
          if (!pinfo.mat4[info.name]) pinfo.mat4[info.name] = { count: 0, unique: new Set(), samples: [] };
          var e = pinfo.mat4[info.name];
          e.count++;
          var key = mat4Key(value);
          if (!e.unique.has(key)) e.unique.add(key);
          if (e.samples.length < 3) e.samples.push({ t: [value[12], value[13], value[14]] });
        }
      } catch(e2) {}
      return origUM4.call(this, loc, transpose, value);
    };
    var origDrawE = Ctor.prototype.drawElements;
    Ctor.prototype.drawElements = function() {
      if (discoveryWindowActive) {
        try { var p = this.getParameter(this.CURRENT_PROGRAM); if (p) getOrMakeProg(p).drawCount++; } catch(e) {}
      }
      return origDrawE.apply(this, arguments);
    };
    var origDrawA = Ctor.prototype.drawArrays;
    Ctor.prototype.drawArrays = function() {
      if (discoveryWindowActive) {
        try { var p = this.getParameter(this.CURRENT_PROGRAM); if (p) getOrMakeProg(p).drawCount++; } catch(e) {}
      }
      return origDrawA.apply(this, arguments);
    };
  }
  installDiscoveryHooks(WebGLRenderingContext, 'w1');
  if (typeof WebGL2RenderingContext !== 'undefined') installDiscoveryHooks(WebGL2RenderingContext, 'w2');
  log('Discovery hooks installed (READ-ONLY, no overrides)');

  function flushDiscoveryReport() {
    if (discoveryFlushed) return;
    discoveryFlushed = true;
    discoveryWindowActive = false;
    logStep(8, '═══ DISCOVERY REPORT (per-program) ═══');
    var progs = Array.from(discoveryPrograms.entries()).sort(function(a,b){ return a[1].index - b[1].index; });
    var report = { programs: [] };
    for (var i = 0; i < progs.length; i++) {
      var info = progs[i][1];
      var cls = classifyProg(info);
      var mat4sum = {};
      for (var mn in info.mat4) {
        var e = info.mat4[mn];
        mat4sum[mn] = { count: e.count, unique: e.unique.size, samples: e.samples };
      }
      var line = '[prog#' + info.index + '] class=' + cls + ' draws=' + info.drawCount +
        ' uniforms=[' + Array.from(info.uniformNames).sort().join(',') + ']' +
        ' mat4=' + JSON.stringify(mat4sum);
      log('[DISCOVERY] ' + line);
      report.programs.push({ index: info.index, class: cls, draws: info.drawCount,
        uniforms: Array.from(info.uniformNames).sort(), shaderUniforms: info.shaderUniforms, mat4: mat4sum });
    }
    remoteLog('DISCOVERY_REPORT', 'Per-program uniform/draw analysis', report);
    logStep(9, 'Discovery report flushed to console + log server. Hooks remain (safe).');
    updateStatus();
  }
  ...
  function startDiscoveryWindow() {
    ...
    log('[DISCOVERY] Window opened — counting draws for 2s...');
    setTimeout(flushDiscoveryReport, 2000);
  }
  ...
  function updateStatus() {
    ...
    var discState = discoveryFlushed ? 'DONE (see console)' : discoveryWindowActive ? 'SAMPLING...' : 'waiting';
    var discLine = '<div style="color:#00ddcc;font-size:10px;margin-top:2px;">DISCOVERY: ' + discState + '</div>';
    overlay.innerHTML = '<div style="font-weight:bold;margin-bottom:4px;color:' + statusColor + ';">Map Simplifier v' + VERSION + '</div>' +
      ...
      '<div style="margin-top:2px;">' + catSwatches + '</div>' + catCountsLine + imgLine + discLine +
