// ==UserScript==
// @name         Tanki Collision Overlay v2.1
// @namespace    https://github.com/0d00no0721/TOandDDD
// @version      2.1.0
// @description  Parse map.bin collision data, render with three.js overlay. Uniform discovery phase. Alt+C toggle or click status panel. Default ON.
...
(function() {
  ...
  function log(m) { console.log('%c[CO v2.1] ' + m, 'color:#00ccff'); remoteLog('INFO', m); }
  function logStep(n, m) { var msg = '[Step ' + n + '] ' + m; console.log('%c[CO v2.1] ' + msg, 'color:#00ff99'); remoteLog('INFO', msg); }
  function logWarn(m) { console.warn('[CO v2.1] ' + m); remoteLog('WARN', m); }
  function logErr(m) { console.error('[CO v2.1] ' + m); remoteLog('ERROR', m); }
  ...
  function updateStatusOverlay() {
    ...
    statusOverlay.innerHTML =
      '<div style="font-weight:bold;margin-bottom:4px;color:' + overlayColor + ';">Collision Overlay v2.1</div>' +
  ...
  function initAfterDeps() {
    ...
    createStatusOverlay();
    log('v2.1 ready — discovery mode active');
  }
  ...
  log('Collision Overlay v2.1 script loaded');
})();