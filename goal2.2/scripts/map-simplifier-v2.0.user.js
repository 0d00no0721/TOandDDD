// ==UserScript==
// @name         Tanki Collision Overlay v2.0
// @namespace    https://github.com/0d00no0721/TOandDDD
// @version      2.0.0
// @description  Parse map.bin collision data, render with three.js overlay. Uniform discovery phase. Ctrl+Shift+C toggle.
...
(function() {
  ...
  function isOverlayEnabled() { try { return localStorage.getItem(OVERLAY_KEY) === 'true'; } catch(e) { return false; } }
  ...
  function log(m) { console.log('%c[CO v2.0] ' + m, 'color:#00ccff'); remoteLog('INFO', m); }
  function logStep(n, m) { var msg = '[Step ' + n + '] ' + m; console.log('%c[CO v2.0] ' + msg, 'color:#00ff99'); remoteLog('INFO', msg); }
  function logWarn(m) { console.warn('[CO v2.0] ' + m); remoteLog('WARN', m); }
  function logErr(m) { console.error('[CO v2.0] ' + m); remoteLog('ERROR', m); }
  ...
  function findGameCanvas() {
    ...
    for (var i = 0; i < canvases.length; i++) {
      if (canvases[i].width > 100 && canvases[i].height > 100) return canvases[i];
    }
    return null;
  }
      ...
      for (var mi = 0; mi < mutations.length; mi++) {
        for (var ni = 0; ni < mutations[mi].addedNodes.length; ni++) {
          ...
          if (node.nodeName === 'CANVAS' || (node.querySelectorAll && node.querySelectorAll('canvas').length > 0)) {
            ...
            if (canvas) { observer.disconnect(); gameCanvas = canvas; callback(); return; }
          }
        }
      }
    });
  ...
  function createStatusOverlay() {
    ...
    statusOverlay.style.cssText = 'position:fixed;top:8px;right:8px;z-index:2147483647;background:rgba(0,10,30,0.92);color:#00ccff;border:1px solid rgba(0,204,255,0.3);border-radius:8px;padding:8px 12px;font-size:12px;font-family:monospace;max-width:380px;pointer-events:none;user-select:none';
  ...
  function updateStatusOverlay() {
    ...
    statusOverlay.innerHTML =
      '<div style="font-weight:bold;margin-bottom:4px;color:' + overlayColor + ';">Collision Overlay v2.0</div>' +
      '<div style="color:#aaa;font-size:10px;">Ctrl+Shift+C toggle | Logs: localhost:3456</div>' +
  ...
  function initAfterDeps() {
    ...
    createStatusOverlay();
    log('v2.0 ready — discovery mode active');
  }
  ...
  log('Collision Overlay v2.0 script loaded');
})();